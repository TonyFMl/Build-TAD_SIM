...
# Install cmake
RUN wget -O cmake-${CMAKE_VERSION}-linux-x86_64.sh https://cmake.org/files/v${CMAKE_VERSION%.*}/cmake-${CMAKE_VERSION}-linux-x86_64.sh \
...
RUN git config --global url."https://github.com/".insteadOf git://github.com/ \
    # && git clone --recurse-submodules -b v${GRPC_VERSION} https://github.com/grpc/grpc.git \
...
# Install Go
COPY go1.17.8.linux-amd64.tar.gz .
...
RUN wget -O Python-${PYTHON_VERSION}.tgz https://www.python.org/ftp/python/${PYTHON_VERSION}/Python-${PYTHON_VERSION}.tgz \
    ...
    && ln -sf /usr/local/bin/python${PIP_VERSION} /usr/local/bin/python3 \
...
# Create user & Change ownership of directory
RUN useradd -ms /bin/bash ${USERNAME} \
    && chown -R ${USERNAME}:${USERNAME} ${WORKDIRNAME} \
...
ENV CMAKE_JS_INC="/home/${USERNAME}/.cmake-js/node-x64/v${NODE_VERSION}/include/node"
ENV LANG=en_US.UTF-8 LANGUAGE=en_US:en LC_ALL=en_US.UTF-8

# config pip
RUN /usr/local/bin/python${PIP_VERSION} -m pip install -i https://mirrors.tencent.com/pypi/simple/ --upgrade pip \
